...
import sys        # proporciona acceso a variables y funciones específicas del sistema
import socket     # necesaria para crear sockets
import optparse   # analizador sintáctico interno
...
def parse_server(url):
    ...
    path = url[len(PREFIX):]
    # Crea una lista con strings contenidos antes de cada " / "
    path_elements = path.split('/')
    # Nos quedamos con el 1ro
...
def send_request(connection, url):
    ...
    # encode() devuelve una versión codificada en utf-8 de la cadena
...
def read_line(connection):
    ...
    # solo si hay un error en try, se ejecutara except
    except:
        error = True
    # mientras no haya errores y el byte recibido no sea ni ' ' ni '\n', vamos concatenando cada caracter en result
...
def check_http_response(header):
    ...
    False
    """
    # decode() devuelve una version decodificada de la cadena
    header = header.decode()
    # divide la cadena por espacios, hasta un maximo de 3 cadenas
