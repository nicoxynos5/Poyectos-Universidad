...
# $exit_code -eq 0 means executed without problems
# $exit_code -eq 1 means executed with problems
...
exit_code=$?
if [ $exit_code -eq 0 ] # ls finds dir4
then
  echo "-------- TEST FAILED: $TEST_DESCRIPTION"
  clean_and_exit -1
else
...
echo ""
TEST_DESCRIPTION="Directory is still deleted (rmdir) after re-mounting" 
echo "... Unmounting and remounting system" 
sleep 1
fusermount -u ${MOUNTING_POINT}
sleep 1  # Make sure it finished mounting
...
echo ""
TEST_DESCRIPTION="Directory is still deleted (rm -r) after re-mounting" 
echo "... Unmounting and remounting system" 
sleep 1
fusermount -u ${MOUNTING_POINT}
sleep 1  # Make sure it finished mounting
...
exit_code=$?
if [ $exit_code -eq 1 ]  # ls does not find dir6
then
  echo "-------- TEST FAILED: $TEST_DESCRIPTION"
  clean_and_exit -1
else
...
exit_code=$?
if [ $exit_code -eq 0 ]  # ls finds dir7
then
  echo "-------- TEST FAILED: $TEST_DESCRIPTION"
  clean_and_exit -1
else
