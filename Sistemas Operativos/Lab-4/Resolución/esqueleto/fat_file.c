 ...
 * function error.
 */
// static void fat_file_print_dentry(fat_file file) {
//     printf("\t file: \n");
//     printf("\t name: %s\n", file->name);
//     printf("\t filepath: %s\n", file->filepath);
...
//     }
//     printf("\t pos_in_parent: %d\n", file->pos_in_parent);
//     printf("\t Dentry: \n");
//     printf("\t Basename: %.*s\n", 8, file->dentry->base_name);
//     printf("\t Extension: %.*s\n", 3,file->dentry->extension);
//     printf("\t Attributes: %x\n", file->dentry->attribs);
...
void fat_file_unlink(fat_file file, fat_file parent) {
    u32 delete_cluster = file->start_cluster;
    u32 next_cluster = 0;

    // Mark following clusters as not used
    while (fat_table_cluster_is_valid(delete_cluster)) {
        // If there was an error, we continue with the function.
        next_cluster = fat_table_get_next_cluster(file->table, delete_cluster);
        fat_table_set_next_cluster(file->table, delete_cluster, FAT_CLUSTER_FREE);
        delete_cluster = next_cluster;
    }
    
    // Update entrance in directory

    file->dentry->base_name[0] = 0xe5;
    fill_dentry_time_now(parent->dentry, false, true);
    write_dir_entry(parent, file);
}

void fat_file_rmdir(fat_file child, fat_file parent) {

    fat_file_unlink(child, parent);
